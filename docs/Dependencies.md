...
| :--- | :--- | :--- |
| [**Cobblemon**](https://modrinth.com/mod/cobblemon) `1.8.0` | The **Party** tab, **Pokémon Skins**, the **Cobblemon Effects / Lure** bonus system, five **scanners** (IV / nature / ability / size / dex), **Heal Ability** and **Shiny & HA Radar**. See [Integrations](Integrations.md). | Those features are hidden. Any Lure/scanner data already saved on a cosmetic is kept in the file and ignored. All other cosmetic features work normally. |
| [**LuckPerms**](https://luckperms.net/) | Permission nodes (per-slot / per-type limits, per-cosmetic `permission`, per-cosmetic **Granted Permissions**, command permissions), and the automatic group-based chat **Tags** tab. See [Integrations](Integrations.md). | Only server operators can use restricted features; the **Tags** tab and the Dev Studio **Chat Tags** page are hidden. **Granted Permissions** fields still save but silently do nothing (no permission plugin to apply them to) — **Minecraft Tags** (vanilla `/tag`) keep working regardless. |
