...
A **group id is used exactly where an effect id is used** (the cosmetic's *Effect Visual* / *Fly Particle* fields). The mod expands it to its members at spawn time; each member keeps its own `tickInterval`.
